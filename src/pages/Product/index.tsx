  ...
  useEffect(() => {
    getProduct(Number(id)).then((productResp) => {
      ...
      getSimilarProducts(productResp.category).then((productList) =>
        setSimilarItemsList(
          productList.filter(
            (productItem: IProduct) => productItem.title != productResp.title,
          ),
        ),
      );
      return;
        ...
        <styled.ProductWrapper>
            ...
            <styled.FastInfoSection>
              <styled.MainInfoWrapper>
                ...
                <styled.RatingContainer>
                  ...
                  <styled.ReviewCounter>
                    {product.rating.count} review
                    {product.rating.count != 1 ? 's' : ''}
                  </styled.ReviewCounter>
                </styled.RatingContainer>

                <styled.ProductDescription isFull={isFull}>
                  {product.description}
                </styled.ProductDescription>
                <styled.ViewMoreButton
                  onClick={() => setIsFull((prevState) => !prevState)}
                >
                  {!isFull ? 'View All' : 'Close'}
                </styled.ViewMoreButton>
              </styled.MainInfoWrapper>
              <styled.ProductCategories>
                <styled.ProductCategoriesTitle>
                  Categories:&nbsp;{' '}
                </styled.ProductCategoriesTitle>
                <styled.ProductCategoriesNames>
                  {product.category}
                </styled.ProductCategoriesNames>
              </styled.ProductCategories>
            </styled.FastInfoSection>
          ...
          <styled.SimilarItemsSection>
            ...
            <styled.SimilarItemsContainer>
              {similarItemsList == null && <h1>No Matches</h1>}
              {similarItemsList != null &&
                similarItemsList
                  .slice(0, 3)
                  .map((product) => <Card {...product}></Card>)}
            </styled.SimilarItemsContainer>
          </styled.SimilarItemsSection>
        </styled.ProductWrapper>
      )}
