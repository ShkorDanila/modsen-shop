...
const ProductPhotoOverview = ({ urls }: IPhotosProps) => {
  ...
  function handleSidePictureClick(iterator: number) {
    ...
    setSidePictures((lastArray) =>
      lastArray.map((item, i) => (i == iterator ? currentPicture : item)),
    );
  }
  ...
  return (
    <PhotosWrapper>
      <SmallPhotos>
        {sidePictures.map((url, iterator) => (
          <PhotoWrapper key={iterator}>
            <SingleSmallPhoto
              onClick={() => handleSidePictureClick(iterator)}
              key={iterator}
              photoId={iterator}
              link={url}
            ></SingleSmallPhoto>
